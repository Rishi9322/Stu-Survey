...
<head>
    ...
    <title>Student Satisfaction Survey System - Future Ready v2.0</title>
    <!-- Three.js and GLTFLoader -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js?v=2025091403"></script>
    <script src="https://cdn.jsdelivr.net/npm/three@0.128/examples/js/loaders/GLTFLoader.min.js"></script>
    <!-- Lenis Smooth Scrolling - Updated reliable CDN -->
    ...
    <style>
        * {
        ...
        /* Enhanced Performance for 3D Elements */
        .canvas-container {
            position: relative;
            height: 600px;
            display: flex;
...
<body>
    ...
    <script>
        // Enhanced 3D Homepage with Lenis Smooth Scrolling
        console.log('3D Homepage v2.0 - GLB Model + Lenis.js Integration');
        console.log('Loading 3D model from assets/52690202-9783-47a3-99c1-80ee3fa1a3fa.glb');
        
        // Initialize Lenis smooth scrolling
        let lenis;
        
        // 3D Scene variables
        let scene, camera, renderer, laptopModel, animationId;
        ...
        function initLenis() {
            ...
            if (typeof Lenis === 'undefined') {
                console.error('Lenis library not loaded. Using native smooth scroll fallback.');
                document.documentElement.style.scrollBehavior = 'smooth';
                
                // Set up native scroll handlers
            ...
            // Initialize Lenis with simpler configuration
            lenis = new Lenis({
                duration: 1.2,
    ...
    <script>
        ...
        // 3D Scene Setup with GLB Model Loading
        function init3D() {
            console.log('Initializing 3D scene with GLB model...');
            ...
            const rimLight = new THREE.DirectionalLight(0x00ff94, 0.3);
        ...
        function loadLaptopModel() {
            ...
            console.log('Attempting to load 3D laptop model...');
            
            loader.load(
                'assets/52690202-9783-47a3-99c1-80ee3fa1a3fa.glb',
                (gltf) => {
                    console.log('✅ 3D laptop model loaded successfully!');
                    laptopModel = gltf.scene;
                    
                    // Enhanced scaling - make it much larger and more prominent
                    laptopModel.scale.setScalar(4);
                    ...
                    laptopModel.traverse((child) => {
                        if (child.isMesh) {
                            ...
                            // Enhance materials for better interactivity
                            if (child.material) {
                ...
                (error) => {
                    ...
                    console.log('🔄 Falling back to procedural laptop model...');
                    createFallbackLaptop();
                }
            );
        }

        function createFallbackLaptop() {
            console.log('✨ Creating fallback procedural laptop model...');
            ...
            laptopModel.add(touchpad);

            // Position the fallback model - match the GLB model scaling
            laptopModel.position.set(0, -1, 0);
            laptopModel.scale.setScalar(2.5);

            scene.add(laptopModel);
            console.log('✅ Fallback laptop model created and added to scene');
        }
        ...
        function animate() {
            ...
            if (laptopModel) {
                // Enhanced smooth rotation based on mouse - more responsive
                const currentRotationY = laptopModel.rotation.y;
                const currentRotationX = laptopModel.rotation.x;
                
                laptopModel.rotation.y += (targetRotationY - currentRotationY) * 0.1;
                laptopModel.rotation.x += (targetRotationX - currentRotationX) * 0.1;

                // Enhanced floating animation - more dynamic
                laptopModel.position.y = -0.5 + Math.sin(Date.now() * 0.002) * 0.4;
                
                // Reduced continuous rotation to make mouse interaction more prominent
                ...
                laptopModel.rotation.z = scrollPercent * Math.PI * 0.25;
                
                // Keep the large scale but add scroll variation
                const baseScale = 4;
                laptopModel.scale.setScalar(baseScale + scrollPercent * 0.5);
                
                // Enhanced camera movement based on scroll
                camera.position.z = 6 - scrollPercent * 1.5;
                camera.position.y = 1 + scrollPercent * 2;
                camera.lookAt(laptopModel.position);
                
                // Dynamic lighting based on scroll and mouse position
                const lights = scene.children.filter(child => child.isLight);
                lights.forEach((light, index) => {
                    if (light.isDirectionalLight || light.isSpotLight) {
                        const intensity = 0.7 + Math.sin(scrollPercent * Math.PI + index) * 0.3;
                        const mouseInfluence = (mouseX + mouseY) * 0.1;
                        light.intensity = Math.max(0.2, intensity + mouseInfluence);
                    }
                });
            }
        ...
        function updateScrollAnimations() {
            ...
            // Parallax effects on floating elements with Lenis integration
            document.querySelectorAll('.floating').forEach((element, index) => {
                const speed = 0.3 + index * 0.1;
                const rotation = scrollPercent * 3;
                const translateY = scrollY * speed * 0.08;
                
                element.style.transform = `translateY(${translateY}px) rotate(${rotation}deg) scale(${1 + scrollPercent * 0.05})`;
            });

            // Enhanced content animations with Lenis smoothness
            document.querySelectorAll('.feature-card, .stat-card, .testimonial-card, .service-card').forEach((card, index) => {
                ...
                if (isVisible) {
                    ...
                    const rotation = (adjustedProgress - 0.5) * 2;
                    const scale = 0.95 + adjustedProgress * 0.05;
                    const opacity = Math.max(0.1, adjustedProgress);
                    
                    card.style.transform = `translateY(${translateY}px) rotate(${rotation}deg) scale(${scale})`;
                    card.style.opacity = opacity;
                }
            });

            // Update scroll progress with Lenis values
